...
//---MySQL----------------------------------------------------------------------
  ...
  MySqlKW: UnicodeString =
    ...
    'ASC,AT,AUTO_INCREMENT,AUTHORS,AUTOEXTEND_SIZE,AVG_ROW_LENGTH,BACKUP,BEFORE,BEGIN,BENCHMARK,BETWEEN,BINLOG,BIT,' +
    'BOTH,BY,CACHE,CALL,CASCADE,CASCADED,CHAIN,CHANGE,CHANGED,CHARACTER,CHARSET,CHECK,' +
    'CHECKSUM,CLIENT,CODE,COLLATE,COLLATION,COLUMN,COLUMNS,COMMENT,COMMIT,' +
    'COMMITTED,COMPLETION,CONCURRENT,CONNECTION,CONSISTENT,CONSTRAINT,' +
    'CONTAINS,CONTENTS,CONTRIBUTORS,CONVERT,CREATE,CROSS,DATA,DATABASE,DATABASES,' +
    'DATAFILE,DEALLOCATE,DEC,DEFAULT,DEFINER,DELAYED,DELAY_KEY_WRITE,DELETE,DESC,' +
    'DETERMINISTIC,DIRECTORY,DISABLE,DISCARD,DESCRIBE,DISTINCT,DISTINCTROW,' +
    'DIV,DROP,DUAL,DUMPFILE,DUPLICATE,EACH,ELSE,ELSEIF,ENABLE,ENCLOSED,END,ENDIF,ENDS,' +
    'ENGINE,ENGINES,ESCAPE,ESCAPED,ERRORS,EVENT,EVENTS,EVERY,EXECUTE,EXISTS,' +
    'EXPANSION,EXPLAIN,EXTENDED,EXTENT_SIZE,FALSE,FAST,FIELDS,FILE,FIRST,FLUSH,FOR,FORCE,FOREIGN,FROM,' +
    'FULL,FULLTEXT,FUNCTION,FUNCTIONS,GLOBAL,GRANT,GRANTS,GROUP,HAVING,HELP,' +
    'HIGH_PRIORITY,HOSTS,IDENTIFIED,IGNORE,INDEX,INFILE,INNER,INITIAL_SIZE,INSERT,' +
    ...
    'LIKE,LIMIT,LINEAR,LINES,LIST,LOAD,LOCAL,LOCK,LOGFILE,LOGS,LONG,LOW_PRIORITY,' +
    ...
    'MAX_ROWS,MAX_SIZE,MAXVALUE,MEDIUM,MIDDLEINT,MIN_ROWS,MOD,MODE,MODIFY,MODIFIES,NAMES,' +
    ...
    'PROCESS,PROCESSLIST,QUERY,QUICK,RAID_CHUNKS,RAID_CHUNKSIZE,RAID_TYPE,RANGE,' +
    ...
    'VARIABLES,VARYING,VIEW,WAIT,WARNINGS,WHERE,WITH,WORK,WRITE';

  // PLSQL keywords
  MySQLPLSQLKW: UnicodeString =
    'CATALOG_NAME,CLASS_ORIGIN,CLOSE,COLUMN_NAME,CONDITION,CONTINUE,' +
    'CONSTRAINT_CATALOG,CONSTRAINT_NAME,CONSTRAINT_SCHEMA,CURSOR,CURSOR_NAME,' +
    'DECLARE,DO,EXIT,FETCH,FOUND,GOTO,HANDLER,ITERATE,LANGUAGE,LEAVE,LOOP,' +
    'MESSAGE_TEXT,MYSQL_ERRNO,SCHEMA_NAME,SQLEXCEPTION,SQLSTATE,SQLWARNING,' +
    'SIGNAL,SUBCLASS_ORIGIN,TABLE_NAME,UNTIL,WHILE';
  ...
  MySQLTypes: UnicodeString =
    ...
    // Column Types
    'bigint,blob,bool,boolean,byte,char,date,datetime,decimal,double,enum,float,' +
  ...
  MySQLFunctions: UnicodeString =
    'ABS,ACOS,ADD,ADDDATE,ADDTIME,AES_DECRYPT,AES_ENCRYPT,ANALYSE,ASCII,ASIN,ATAN,ATAN2,AVG,BIN,BIT_AND,' +
    'BIT_COUNT,BIT_LENGTH,BIT_OR,BIT_XOR,CASE,CAST,CEIL,CEILING,CHARACTER_LENGTH,' +
    ...
    'RPAD,RTRIM,SECOND,SEC_TO_TIME,SESSION_USER,SIGN,SIN,SLEEP,SOUNDEX,SPACE,' +
