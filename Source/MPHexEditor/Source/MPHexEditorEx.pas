...
{$IFDEF DELPHI6UP}{$IFDEF USESTDACTIONS}
...
procedure MPHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName:
      string; TextAsHex, SwapNibbles: boolean);
...
type
  ...
  public
    ...
    constructor CreateFromStream(Stream: TStream; Position, DataSize: integer;
      ScrapFileName: string; TextAsHex, SwapNibbles: boolean);
    procedure BeforeDestruction; override;
...
{$IFDEF DELPHI6UP}{$IFDEF USESTDACTIONS}
function TMPHexEditorEx.ExecuteAction(Action: TBasicAction): Boolean;
begin
...
function TMPHexEditorEx.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := Focused;
  if Result then
  begin
    ...
    begin
      if Action is TEditCut then
        TEditAction(Action).Enabled := CanCut
      else if Action is TEditCopy then
        TEditAction(Action).Enabled := CanCopy
      else if Action is TEditPaste then
        TEditAction(Action).Enabled := CanPaste
      else if Action is TEditDelete then
        TEditAction(Action).Enabled := CanCut
      else if Action is TEditUndo then
        TEditUndo(Action).Enabled := CanUndo
      else if Action is TEditSelectAll then
        TEditSelectAll(Action).Enabled := DataSize > 0
      else
        Result := False;
    end
    else
      ...
      for LIntCount := 0 to Pred(LIntCount) do
        with LPtrProps^[LIntCount]^ do
          if PropType^^.Kind = tkClass then
            Recurse(GetObjectProp(Ref, string(Name)), Prefix + string(Name) + '.')
          else if IsPropPublic(Prefix + string(Name)) then
            sl.Add(Prefix + string(Name) + '=' +string(GetPropValue(Ref, string(Name){$IFDEF DELPHI7UP}, False{$ENDIF})));
    ...
    try
      ...
      if Count > 0 then
        ...
        begin
          ...
          begin
{$IFNDEF DELPHI7UP}
            LStrVal := ValueFromIndex(LStrData, LIntLoop);
{$ELSE}
...
constructor TMPHDataObject.CreateFromStream(Stream: TStream; Position, DataSize:
  integer; ScrapFileName: string; TextAsHex, SwapNibbles: boolean);
begin
  inherited Create;
  FData := nil;
  FHasDropEffect := False;
...
begin
  ...
  begin
    ...
    if formatetcIn.cfFormat = CF_REGEDIT_HEXDATA then
      LIntDataSize := sizeof(TRegEditHexData) - 1 + FDataSize
    ...
    begin
      ...
      try
        try
          ...
          else if formatetcIn.cfFormat = CF_REGEDIT_HEXDATA then
          begin
            Pointer(LPtrRegData) := Pointer(LPtrLocal);
            LPtrRegData^.Size := FDataSize;
            Move(FData^, LPtrRegData^.Data, FDataSize);
          end
          else if formatetcIn.cfFormat = CF_FILEDESCRIPTOR then
          begin
...
begin
  ...
  begin
    if dwAspect <> DVASPECT_CONTENT then
      ...
      (cfFormat = CF_FILEDESCRIPTOR) or (cfFormat = CF_FILECONTENTS) or (cfFormat = CF_REGEDIT_HEXDATA) then
      Result := S_OK;
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Pos(LWChrText, FEditor.MaskedChars) > 0)
            then
      ...
      begin
        ...
        begin
          ...
          if FEditor.UnicodeChars then
            while (FCanvas.Font.Size > 1) and GetTextExtentPoint32W(FCanvas.Handle, @LRecTextAttr.Text[LIntLoop],
...
procedure MPHSetHexClipboardData(Data: Pointer; DataSize: integer; ScrapFileName:
      string; TextAsHex, SwapNibbles: boolean);
begin
  OleCheck(OleSetClipboard(TMPHDataObject.Create(Data, DataSize, ScrapFileName,
      TextAsHex, SwapNibbles)));
end;
