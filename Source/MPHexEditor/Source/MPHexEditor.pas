(*
  ...
  <li>v 09-30-2007: september 30, 2007<br><br>
         - different calculation of textheight and charwidth to better support
           e.g. chinese unicode characters<br>
         - When changing @link(HexLowerCase), the ruler has not been drawn
           correctly<br><br></li>

  <li>v 12-06-2006: december 06, 2006<br><br>
         - changed SelectionChanged method, so the @link(OnSelectionChanged) event should work when
           the editor control is put on a TFrame component<br>
         - added delphi2006 package<br>
         - added @link(AutoBytesPerRow) property<br>
         - the @link(SelCount) property is writeable now<br>
         - standard actions support in @link(TMPHexEditorEx)<br><br></li>

  <li>v 05-24-2006: may 24, 2006<br><br>
         - fixed painting bug when drawing solid "secondary" cursor frames (if @link(FocusFrame) = False)<br>
...
type
  ...
  TMPHCharConvTable = array[0..255] of AnsiChar;
...
function TCustomMPHexEditor.TranslateFromAnsiChar(const aByte: byte): WideChar;
begin
  case FTranslation of
    ...
    tkMac: Result := WideChar(MPH_CCONV_MAC[cctFromAnsi][aByte]);
    tkBCD: Result := WideChar(MPH_CCONV_BCD38[cctFromAnsi][aByte]);

    tkCustom: Result := WideChar(MPHCustomCharConv[cctFromAnsi][aByte]);
...
function TCustomMPHexEditor.TranslateToAnsiChar(const aByte: byte): WideChar;
begin
  case FTranslation of
    tkAsIs: Result := WideChar(aByte);
    tkDos8,
    ...
    tkMac: Result := WideChar(MPH_CCONV_MAC[cctToAnsi][aByte]);
    tkBCD: Result := WideChar(MPH_CCONV_BCD38[cctToAnsi][aByte]);

    tkCustom: Result := WideChar(MPHCustomCharConv[cctToAnsi][aByte]);
...
procedure TCustomMPHexEditor.WMImeChar(var Msg: TWMChar);
begin
  WMChar(Msg);
end;
...
procedure TCustomMPHexEditor.SetSelCount(aValue: integer);
begin
...
procedure TCustomMPHexEditor.SetAsText(const Value: AnsiString);
var
...
function TCustomMPHexEditor.GetAsText: AnsiString;
begin
  if DataSize < 1 then
    Result := ''
  else
...
procedure TCustomMPHexEditor.RecalcBytesPerRow;
...
procedure TCustomMPHexEditor.SetAutoBytesPerRow(const Value: Boolean);
begin
...
function TCustomMPHexEditor.GetSelectionAsText: AnsiString;
begin
  if (DataSize < 1) or (SelCount < 1) then
    Result := ''
  else
...
procedure TCustomMPHexEditor.SetSelectionAsText(const s: AnsiString);
begin
  if s <> '' then
...
procedure TCustomMPHexEditor.SetMaskedChars(const Value: string);
begin
...
begin
  ...
  begin
    MPHCustomCharConv[cctFromAnsi][LBytLoop] := AnsiChar(LBytLoop);
    MPHCustomCharConv[cctToAnsi][LBytLoop] := AnsiChar(LBytLoop);
  end;
end;
...
function TMPHMemoryStream.PointerAt(const APosition, ACount: Integer): Pointer;
begin
