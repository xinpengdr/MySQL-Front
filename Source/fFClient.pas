...
type
  ...
  private
    ...
    procedure ListViewRefresh(const ClientEvent: TCClient.TEvent; const Kind: TADesktop.TListViewKind; const ListView: TListView; const Data: TCustomData = nil);
...
begin
  ...
  begin
    ...
    case (View) of
      ...
      vBrowser: NewActiveControl := FGrid;
    ...
    UsedView := View;
    case (View) of
      vObjects: if (not (ttObjectBrowser in Preferences.ToolbarTabs)) then begin Include(Preferences.ToolbarTabs, ttObjectBrowser); PostMessage(Window.Handle, CM_CHANGEPREFERENCES, 0, 0); end;
      vBrowser: if (not (ttDataBrowser in Preferences.ToolbarTabs)) then begin Include(Preferences.ToolbarTabs, ttDataBrowser); PostMessage(Window.Handle, CM_CHANGEPREFERENCES, 0, 0); end;
      vIDE: if (not (ttObjectIDE in Preferences.ToolbarTabs)) then begin Include(Preferences.ToolbarTabs, ttObjectIDE); PostMessage(Window.Handle, CM_CHANGEPREFERENCES, 0, 0); end;
      vBuilder: if (not (ttQueryBuilder in Preferences.ToolbarTabs)) then begin Include(Preferences.ToolbarTabs, ttQueryBuilder); PostMessage(Window.Handle, CM_CHANGEPREFERENCES, 0, 0); end;
      vEditor: if (not (ttSQLEditor in Preferences.ToolbarTabs)) then begin Include(Preferences.ToolbarTabs, ttSQLEditor); PostMessage(Window.Handle, CM_CHANGEPREFERENCES, 0, 0); end;
    ...
    begin
      ...
      if ((UsedView = vBrowser) and (SelectedTable <> '')) then
      begin
    ...
    begin
      ...
      begin
        ...
        case (View) of
          ...
          vBrowser: if (PResult.Visible and PGrid.Visible) then Window.ActiveControl := FGrid;
          vIDE: if (PSQLEditor.Visible and Assigned(ActiveSynMemo)) then Window.ActiveControl := ActiveSynMemo;
          vBuilder: if (PBuilder.Visible and Assigned(FBuilderActiveWorkArea())) then Window.ActiveControl := FBuilderActiveWorkArea();
          vEditor: if (PSQLEditor.Visible) then Window.ActiveControl := FSQLEditorSynMemo;
...
begin
  ...
  if (Assigned(ClientEvent)) then
    if (ClientEvent.EventType in [ceBuild, ceObjBuild, ceObjCreated, ceObjDroped, ceObjAltered]) then
  ...
  begin
    ...
    begin
      ...
      if ((ClientEvent.EventType in [ceObjCreated, ceObjDroped]) and (ClientEvent.CObject is TCTrigger)) then
        ListViewRefresh(ClientEvent, lkTable, TTableDesktop(TCTrigger(ClientEvent.CObject).Table.Desktop).ListView);
    end
...
begin
  ...
  if ((View = vBrowser) and (DataSet is TCTableDataSet) and (TCTableDataSet(DataSet).DatabaseName = SelectedDatabase) and (TCTableDataSet(DataSet).CommandText = SelectedTable)) then
  begin
...
begin
  ...
  ListView.HelpContext := FServerListView.HelpContext;
  ListView.Align := FServerListView.Align;
  ListView.BorderStyle := FServerListView.BorderStyle;
  ListView.DragMode := FServerListView.DragMode;
  ListView.HideSelection := FServerListView.HideSelection;
  ListView.MultiSelect := FServerListView.MultiSelect;
  ListView.GroupView := FServerListView.GroupView;
  ListView.PopupMenu := FServerListView.PopupMenu;
  ListView.RowSelect := FServerListView.RowSelect;
  ListView.SmallImages := Preferences.SmallImages;
  ListView.Parent := FServerListView.Parent;
  ListView.ViewStyle := FServerListView.ViewStyle;
  ListView.Visible := False;
  ListView.OnChange := FServerListView.OnChange;
  ListView.OnChanging := FServerListView.OnChanging;
  ListView.OnColumnClick := FServerListView.OnColumnClick;
  ListView.OnCompare := FServerListView.OnCompare;
  ListView.OnDblClick := FServerListView.OnDblClick;
  ListView.OnEdited := FServerListView.OnEdited;
  ListView.OnEditing := FServerListView.OnEditing;
  ListView.OnEnter := FServerListView.OnEnter;
  ListView.OnExit := FServerListView.OnExit;
  ListView.OnDragDrop := FServerListView.OnDragDrop;
  ListView.OnDragOver := FServerListView.OnDragOver;
  ListView.OnKeyDown := FServerListView.OnKeyDown;
  ListView.OnSelectItem := FServerListView.OnSelectItem;
...
begin
  ...
  PSQLEditor.Update(); // TSynMemo aktualisiert leider nicht sofort nach Änderung von TSynMemo.Align
...
begin
  ...
  if ((DataSet.FieldCount > 0)
    and ((View = vBrowser) and (DataSet is TCTableDataSet) and (TCTableDataSet(DataSet).DatabaseName = SelectedDatabase) and (TCTableDataSet(DataSet).CommandText = SelectedTable)
      or (View = vIDE) and Assigned(ResultSet) and (ResultSet <> Client.QueryBuilderResult) and (ResultSet <> Client.SQLEditorResult)
      or (View = vBuilder) and (ResultSet = Client.QueryBuilderResult)
      or (View = vEditor) and (ResultSet = Client.SQLEditorResult))) then
  begin
...
begin
  ...
  begin
    ...
    else if ((Column.Field.Tag and ftSortedField <> 0) and (not CheckWin32Version(6) or StyleServices.Enabled) and not CheckWin32Version(6, 1)) then
    ...
    begin
      ...
      begin
        ...
        if (not Assigned(Database)) then
          ...
          Table := Database.TableByName(SelectedTable); // Must be first für Views where FieldInfo.TableName the name of the BaseTable
...
begin
  ...
  begin
    ...
    else if (AllowExpansion and Node.HasChildren and (Node.Count = 0)) then
    begin
      FNavigatorNodeToExpand := Node;
      if (Assigned(Table)) then
...
procedure TFClient.FNavigatorRefresh(const ClientEvent: TCClient.TEvent);
  ...
  procedure UpdateGroup(const Node: TTreeNode; const CItems: TCItems);
  var
    Child: TTreeNode;
  ...
  begin
    case (ClientEvent.EventType) of
      ...
      ceObjBuild:
        begin
          for I := Node.Count - 1 downto 0 do
            if ((TCItem(Node.Data).CItems = CItems) and (CItems.IndexOf(Node.Item[I].Data) < 0)) then
            begin
      ...
      ceObjCreated:
        InsertChild(Node, ClientEvent.CObject);
      ceObjAltered:
        begin
          Child := Node.getFirstChild();
          while (Assigned(Child) and (Child.Data <> ClientEvent.CObject)) do
            Child := Child.getNextSibling();
          if (Assigned(Child) and (Child.Text <> ClientEvent.CObject.Caption)) then
          begin
            Child.Text := ClientEvent.CObject.Caption;
      ...
      ceObjDroped:
        for I := Node.Count - 1 downto 0 do
          if (Node.Item[I].Data = ClientEvent.CObject) then
          begin
            if (Node.Item[I] = FNavigatorNodeToExpand) then
              FNavigatorNodeToExpand := nil;
            Node.Item[I].Delete();
...
begin
  ...
  begin
    ...
    if (ClientEvent.CItems is TCDatabases) then
      UpdateGroup(Node, ClientEvent.CItems);
  end
      ...
      begin
        for I := 0 to Node.Count - 1 do
          if (Node.Item[I].ImageIndex = iiBaseTable) then
            UpdateGroup(Node.Item[I], ClientEvent.CItems);
      end;
    end;
  end
  else if (ClientEvent.Sender is TCTable) then
    ...
    begin
      ...
      begin
        ...
        if (Table is TCBaseTable) then
          UpdateGroup(Node, TCBaseTable(Table).Indices);
        UpdateGroup(Node, Table.Fields);
        if ((Table is TCBaseTable) and Assigned(TCBaseTable(Table).ForeignKeys)) then
          UpdateGroup(Node, TCBaseTable(Table).ForeignKeys);
        if ((Table is TCBaseTable) and Assigned(Table.Database.Triggers)) then
          UpdateGroup(Node, Table.Database.Triggers);
    ...
    case (FNavigator.Selected.ImageIndex) of
      ...
      iiHosts:
        begin
          ...
          begin
            CreateListView(Client, Client.Hosts, HostsListView);
      ...
      iiProcesses:
        begin
          ...
          begin
            CreateListView(Client, Client.Processes, ProcessesListView);
      ...
      iiStati:
        begin
          ...
          begin
            CreateListView(Client, Client.Stati, StatiListView);
      ...
      iiUsers:
        begin
          ...
          begin
            CreateListView(Client, Client.Users, UsersListView);
      ...
      iiVariables:
        begin
          ...
          begin
            CreateListView(Client, Client.Variables, VariablesListView);
...
begin
  if ((View = vIDE) and (SelectedImageIndex = iiProcedure)) then
  ...
  else if (View = vBuilder) then
    Result := Client.QueryBuilderResult
  else if (View = vEditor) then
...
begin
  if (MainAction('aVObjectBrowser').Checked) then Result := vObjects
  else if (MainAction('aVDataBrowser').Checked) then Result := vBrowser
  else if (MainAction('aVObjectIDE').Checked) then Result := vIDE
  else if (MainAction('aVQueryBuilder').Checked) then Result := vBuilder
  else if (MainAction('aVSQLEditor').Checked) then Result := vEditor
  else if (MainAction('aVDiagram').Checked) then Result := vDiagram
...
procedure TFClient.ListViewChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
var
  I: Integer;
  List: TList;
begin
  if ((Sender is TListView) and not Client.InUse) then
  begin
    List := TList.Create();
    for I := 0 to TListView(Sender).Items.Count - 1 do
      if (TListView(Sender).Items[I].Selected and (TObject(TListView(Sender).Items[I].Data) is TCObject)) then
        List.Add(TCObject(TListView(Sender).Items[I].Data));
    Client.Update(List, True);
    List.Free();
  end;
end;
...
procedure TFClient.ListViewRefresh(const ClientEvent: TCClient.TEvent; const Kind: TADesktop.TListViewKind; const ListView: TListView; const Data: TCustomData = nil);
  ...
  begin
    case (ClientEvent.EventType) of
      ...
      ceObjBuild:
        begin
          for I := ListView.Items.Count - 1 downto 0 do
            if ((TCItem(ListView.Items[I].Data).CItems = CItems) and (CItems.IndexOf(ListView.Items[I].Data) < 0)) then
      ...
      ceObjCreated:
        begin
          Item := InsertItem(ClientEvent.CObject);
      ...
      ceObjAltered:
        begin
          Index := 0;
          while ((Index < ListView.Items.Count) and (ListView.Items[Index].Data <> ClientEvent.CObject)) do
            Inc(Index);
          if (Index = ListView.Items.Count) then
            InsertItem(ClientEvent.CObject)
          else if (ListView.Items[Index].Caption = ClientEvent.CObject.Caption) then
            UpdateItem(ListView.Items[Index], ClientEvent.CObject)
          else
          begin
      ...
      ceObjDroped:
        for I := ListView.Items.Count - 1 downto 0 do
          if (ListView.Items[I].Data = ClientEvent.CObject) then
            ListView.Items.Delete(I);
      ceObjStatus:
        for I := 0 to ListView.Items.Count - 1 do
          if (ListView.Items[I].Data = ClientEvent.CObject) then
            UpdateItem(ListView.Items[I], ClientEvent.CObject);
    end;

    if (ClientEvent.EventType in [ceBuild, ceObjBuild, ceObjCreated, ceObjDroped]) then
      case (GroupID) of
        giDatabases:
        ...
        giTriggers:
          begin
            ...
            for I := 0 to TCTriggers(CItems).Count - 1 do
              ...
              begin
                InsertItem(TCTriggers(CItems)[I]);
                Inc(Count);
              end;
    ...
    case (Kind) of
      lkServer:
        begin
          ...
          begin
            if (Assigned(Client.Hosts)) then
              InsertItem(Client.Hosts);
            if (Assigned(Client.Processes)) then
              InsertItem(Client.Processes);
            if (Assigned(Client.Stati)) then
              InsertItem(Client.Stati);
            if (Assigned(Client.Users)) then
              InsertItem(Client.Users);
            if (Assigned(Client.Variables)) then
              InsertItem(Client.Variables);
            ListViewInitialize(ClientEvent, ListView, Data);
          end;
      ...
      lkTable:
        ...
        else if ((ClientEvent.Sender is TCDatabase) and (ClientEvent.CObject is TCTrigger)) then
          UpdateGroup(giTriggers, ClientEvent.CItems);
      lkHosts:
...
begin
  ...
  begin
    PResultVisible := True;
    if ((View = vBrowser) and Assigned(Table)) then
      SetDataSource(nil, Table.DataSet)
    else if (View = vIDE) then
      case (SelectedImageIndex) of
    ...
    if ((View = vIDE) and (Assigned(Routine) and Assigned(Routine.InputDataSet) or Assigned(Trigger) and Assigned(Trigger.InputDataSet))) then
    begin
      PObjectIDE.Top := 0;
    ...
    begin
      TCResult.Visible := (View in [vIDE, vEditor]) and (TCResult.Tabs.Count > 1);
...
begin
  ...
  if ((View = vBrowser) and (SelectedImageIndex in [iiBaseTable, iiSystemView, iiView])) then
  begin
    if (Assigned(FGrid.DataSource.DataSet)) then
      FGridRefresh(Sender);
  end
  else if ((View in [vIDE, vBuilder, vEditor]) and Assigned(FGrid.DataSource.DataSet) and FGrid.DataSource.DataSet.Active) then
    FGridRefresh(Sender);
...
begin
  ...
  begin
    ...
    begin
      if ((Length(ResultSet.RawDataSets) = 1) and Assigned(Client.Account.HistoryXML)) then
      begin
...
begin
  ...
  begin
    ...
    Count := -1;
    case (View) of
      vObjects: Count := ActiveListView.Items.Count;
      vBrowser: if (Assigned(FGrid.DataSource.DataSet) and FGrid.DataSource.DataSet.Active) then Count := FGrid.DataSource.DataSet.RecordCount;
      vIDE,
      vEditor:
    ...
    else if ((View = vBrowser) and (FGrid.DataSource.DataSet is TMySQLTable) and not Client.InUse() and Assigned(Table) and TMySQLTable(FGrid.DataSource.DataSet).LimitedDataReceived and (Table.Rows >= 0)) then
...
begin
  ...
  case (View) of
    ...
    vEditor:
      case (SelectedImageIndex) of
        iiServer:
          if (Client.Account.Connection.Prefetch > 0) then
            begin
              List := TList.Create();
              for I := 0 to Client.Databases.Count - 1 do
    ...
    vBrowser:
      if (not TCTable(FNavigator.Selected.Data).ValidDataSet) then
        TableOpen(nil);
  end;
end;
