...
type
  ...
  protected
    ...
    TimeDiff: TDateTime;
    function Error(const AHandle: MySQLConsts.MYSQL): string;
    ...
    function SyncExecuteSQL(const SynchroThread: TSynchroThread; Synchron: Boolean = False): Boolean; virtual;
...
function TMySQLConnection.GetErrorMessage(): string;
begin
  if (not Assigned(Lib) or not Assigned(Handle)) then
    Result := ''
    ...
    try
      if (BitField(Field)) then
        ...
        case (Field.DataType) of
          ...
          ftLongWord: begin SetString(S, Data^.LibRow^[Field.FieldNo - 1], Data^.LibLengths^[Field.FieldNo - 1]); LongWord(Buffer^) := StrToInt(S); end;
          ftLargeint:
...
begin
  ...
  begin
    ...
    try
      Success := Connection.ExecuteSQL(SQL);
      if (not Success) then
        raise EMySQLError.Create(Connection.Error(Connection.Handle), Connection.Lib.mysql_errno(Connection.Handle), Connection)
